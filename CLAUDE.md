...
- `udstilling-og-salg.html` — active shops and upcoming markets; reads from `_data/venues.yml` (filtered by `active: true`, split by `type: shop`/`type: market`)
...
- `tidligere_udstillinger.html` — grid list of past venues; reads from `_data/venues.yml` (filtered by `active: false`, rendered in YAML order — newest entries at the top of the past block)
...
**Map:** `map.html` uses Leaflet.js (CDN) with OpenStreetMap tiles. Markers are auto-generated from `_data/venues.yml` (filtered by `active: true`) — addresses are geocoded via Nominatim with localStorage caching, or use explicit `lat`/`lng` fields if present. Overlapping markers are spiderfied via `overlapping-marker-spiderfier-leaflet`. The `featured: true` venue (Eva Kobels butik) gets an orange pin.

**Venue data:** All shops, markets, and past venues live in a single `_data/venues.yml`. Each entry has an `active` flag — `true` shows under "Udstilling og salg" (subgrouped by `type: shop`/`type: market`), `false` shows under "Tidligere udstillinger" with only `name`/`url`/`description` rendered. The map only shows `active: true` entries.
